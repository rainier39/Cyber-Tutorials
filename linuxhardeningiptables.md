...
iptables is a relatively simple rule-based firewall program that essentially ties in to the Linux kernel's networking in order to filter network traffic. iptables has some state management capabilities which we will be using later on in the tutorial. I.E. iptables keeps track of ongoing network connections and is aware of when a connection is brand new, incoming, outgoing, etc. Others may prefer to use programs such as firewalld (Fedora/Red Hat Linux) or ufw (Ubuntu) which use iptables or nftables as their backend. This tutorial covers iptables specifically and exclusively.
